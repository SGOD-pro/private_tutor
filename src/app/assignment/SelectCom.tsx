...
function SelectCom({ batchId }: { batchId: React.MutableRefObject<string|null> }) {
  const subjects = useSelector((state: RootState) => state.Subjects.allSubjects)
    .map((subject) => ({
  ...
  const [selectedSubject, setSelectedSubject] = useState<BatchInterface | null>(null);
  const [batch, setBatch] = useState<BatchInterface | null>(null);

  const handleSubjectChange = useCallback((e: DropdownChangeEvent) => {
  ...
  return (
    <div className="flex gap-3">
      ...
      />
      <div className="card flex justify-content-center min-w-44">
            <Calendar value={date} onChange={(e) => setDate(e.value)} minDate={minDate} maxDate={maxDate} readOnlyInput placeholder="Subbmission date"/>
        </div>
    </div>
  );
}
